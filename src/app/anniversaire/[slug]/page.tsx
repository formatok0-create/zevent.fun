...
/* Même règle que le mariage : rendu à la demande. Un cache, même
   court, servait un 404 aux invités pendant la minute qui suivait
   la publication. */
export const dynamic = "force-dynamic";

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const invitation = await getPublishedInvitation(slug, "anniversaire");

  if (!invitation) {
    return { title: "Invitation introuvable", robots: { index: false } };
  }

  const name = invitation.celebrant_name ?? "Anniversaire";
  const age = invitation.celebrant_age;
  const title = age != null ? `${name} a ${age} ans` : name;
  const date = formatWeddingDateLong(invitation.wedding_date);
  const description = invitation.venue
    ? `${date} — ${invitation.venue}. Venez fêter ça avec nous.`
    : `${date}. Venez fêter ça avec nous.`;
  const base = await getSiteUrl();
  const url = `${base}/anniversaire/${slug}`;
...
export default async function PublicBirthdayPage({ params }: PageProps) {
  const { slug } = await params;
  /* Le produit est vérifié dans la requête : cette route ne peut
     pas afficher un mariage, même si le slug existe. */
  const invitation = await getPublishedInvitation(slug, "anniversaire");
  if (!invitation) {
